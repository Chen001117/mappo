    ...
    def get_env_fn(rank):
        def init_env(rank):
            if all_args.env_name == "MuJoCo":
                env = MujocoEnv(all_args.num_agents, rank) #all_args.seed+rank*1000) #gym.make(all_args.scenario_name) #(all_args)
                env = MARLWrapper(env)
            else:
...
def make_eval_env(all_args):
    def get_env_fn(rank):
        def init_env(rank):
            if all_args.env_name == "MuJoCo":
                env = MujocoEnv(all_args.num_agents)#seed=all_args.seed*50000+rank*10000) #gym.make(all_args.scenario_name) #(all_args)
                env = MARLWrapper(env)
            else:
                print("Can not support the " +
                      all_args.env_name + "environment.")
...
def parse_args(args, parser):
    parser.add_argument('--scenario_name', type=str,
                        default='walker2d-v3', help="Which scenario to run on")
